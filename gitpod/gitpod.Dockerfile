...
RUN yes | pacman --noconfirm -S sudo git-lfs docker
# install gitpod user
...
# random utilities
RUN yes | pacman --noconfirm -S python nodejs npm neovim git unzip gcc docker

# setup nvim
RUN git clone https://github.com/cultlead3r/nvim /home/gitpod/.config/nvim
RUN nvim --headless +PackerSync +qall
# RUN nvim --headless -c 'autocmd User PackerComplete quitall' -c 'PackerSync' -c 'LspInstall pyright'
